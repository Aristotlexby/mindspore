...
def CalLayerTimeCosts(config):
    ...
    for i in range(num_gpus):
        cost = []
        # 读入timeline文件内容
        gpu_timeline_file = r"C:\Users\Administrator\Documents\WeChat Files\wxid_joifc9jyf5ne22\FileStorage\File\2024-03\gpu_timeline_display_0.json"
        ...
        for item in data:
            ...
            if cur_layer == str(num_layers-1) + "-" + layer_name:
                if item.get("name") == "Gradients" and item.get("scope_level") == 0:
                    flag = False
            if not flag:
...
def profile(args):
    ...
    save_file = f"D:/work/mindspore/{config['model_name']}_{config['num_layers']}_{config['mp_degree']}.npy"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name', type=str, required=True, choices=["gpt", "bert", "t5"])
